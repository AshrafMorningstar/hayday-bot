...
VALID_FRIDA_SHA256 = {
    FRIDA_SHA256,
    "b34a33bdbcd2f737ef59811ca57a4c2675a528b7ffae71acd4c35337d3cb93f5",  # 17.17.0 x86_64
    "63fed00f7cb418d28fea35d92d7b290f266688288258e7ec1f03202b9adca243",  # 17.17.0 x86
}
...
class NXRTHConsole:
    ...
    def prepare_assets(self):
        ...
        if server_digest not in VALID_FRIDA_SHA256:
            raise LoaderError(
                f"vault injector SHA-256 mismatch: unexpected {server_digest}"
            )
    ...
    def run(self):
        exit_code = 1
        try:
            # Force Quago blocking when running in auto mode.
            if self.auto_mode:
                os.environ.setdefault("NX_QUAGO", "1")
            self.setup_adb()
            self.prepare_assets()
            ...
            # --- autonomous startup block ---
            if self.auto_mode:
                print("\n[AUTO] ===== AUTO-START MODE =====")
                print(f"[AUTO] Quago blocking: {'enabled' if os.environ.get('NX_QUAGO') == '1' else 'disabled'}")
                print("[AUTO] Loading native engine...")
                try:
                    with self._cmd_lock:
                        self.cmd_loadnative([])
                except LoaderError as _ae:
                    print(f"[AUTO] loadnative failed: {_ae}")
                    raise
                if not self._farm_thread or not self._farm_thread.is_alive():
                    self._farm_stop.clear()
                    self._farm_thread = threading.Thread(
                        target=self._farm_worker,
                        args=(self.auto_wait, self.auto_crop),
                        daemon=True, name="nxrth-farm",
                    )
                    self._farm_thread.start()
...
def _parse_args():
    ...
    p.add_argument(
        "--auto",
        action="store_true",
        default=os.environ.get("NX_AUTO") == "1",
        help="Fully autonomous mode (force Quago block, loadnative, nfarm at startup).",
    )
    p.add_argument(
        "--auto-wait",
        type=int,
        default=int(os.environ.get("NX_AUTO_WAIT") or 130),
        metavar="SECS",
        help="Seconds between farm cycles in auto mode (default 130).",
    )
    p.add_argument(
        "--auto-crop",
        type=int,
        default=int(os.environ.get("NX_AUTO_CROP") or 400001),
        metavar="ID",
